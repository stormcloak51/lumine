...
export const useComment = (comment: TCommentResponse, role: CommentRoles) => {
  ...
  const [isLiked, setLiked] = useState(
    comment?.Like?.some((like) => like.userId === userId) ?? false
  )
  ...
  const likeCommentMutation = useMutation({
    ...
    onSuccess: async (data) => {
      console.log(data, comment, comment.parentId, 'lal')
      const queryFilter: QueryFilters = {
        queryKey:
          role === CommentRoles.MAINCOMMENT
            ? ['comments', comment.postId]
            : ['subComments', comment.postId, comment.parentId],
      }
        ...
        (oldData): InfiniteData<TPaginatedResponse<TCommentResponse>> => {
          ...
          return {
            ...
            pages: oldData.pages.map(
              (page): TPaginatedResponse<TCommentResponse> => {
                return {
                  ...page,
