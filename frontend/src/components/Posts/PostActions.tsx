...
export const PostActions = ({ post, onClickComment, commentsCount }: IPostActions) => {
	...
	const [isInitiallyLiked, setIsInitiallyLiked] = useState(
		post?.Like?.some(like => like.userId === user?.id) ?? false,
	)
	...
	const likePostMutation = useMutation({
		mutationFn: (data: { postId: number; user: IUserCredentials }) => postService.like(data),
	...
	const unlikePostMutation = useMutation({
		mutationFn: (data: { postId: number; user: IUserCredentials }) => postService.unlike(data),
		onMutate: async ({ postId }) => {
			setLocalLikes(prev => prev - 1)
