import { userSelect } from '@/src/infrastructure/config/constants/user.constants'
...
import {
  ...
  EditPostDto,
  UpsertDraftDto,
} from 'src/infrastructure/dtos/post.dto'
...
export class PostService {
  ...
  async delete(postId: number, userId: string ) {
    ...
    try {
      // Use a transaction to delete all related comments first, then the post
      return await this.prisma.$transaction(async (prisma) => {
        ...
        return await prisma.postModel.delete({
          where: {
            id: postId,
          },
        });
      ...
      throw new BadRequestException('Failed to delete post and its related data');
    }
  }
